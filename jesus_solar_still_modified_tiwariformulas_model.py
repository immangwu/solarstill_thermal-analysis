...
def hcw_newCN(C,n,T_w,T_g,d_f):
    Temp=(T_w+T_g)
    density = 353.44/(Temp+273.15);
    SHC = 999.2+(0.1434*Temp)+((1.101*10**-4)*Temp**2)-((6.7581*10**-8)*Temp**3);
    TC_K=0.0244+(0.7673*10**-4)*Temp # thermal Conductivity
    viscos=(1.718*10**-5)+(4.620*10**-8)*Temp
    P_w=PT(T_w);
    P_g=PT(T_g);
    delT=abs(T_w-T_g)+(((P_w-P_g)*(T_w+273))/((2.689*10**5)-P_w)); #https://sci-hub.se/https://www.sciencedirect.com/science/article/abs/pii/S2352152X20309543
    exp_fac=1/(Temp+273.15)
    g=9.81
    Gr=(g*exp_fac*(density**2)*(d_f **3)*delT)/(viscos**2)
    Pr= (viscos*SHC)/TC_K
    hcw_v=(C*(Gr*Pr)**n)*(TC_K/d_f)
    return(hcw_v)
#Evaporative loss coefficient hew

def hew(C,n,T_w,T_g,d_f):
##    if abs(T_w-T_g)==0:
##        hew=0
##    else:
    #print(T_w,T_g)
    P_w = PT(T_w)
    P_g = PT(T_g)
    hcw=hcw_newCN(C,n,T_w,T_g,d_f)
    print(hcw)
    #print(hcw)
    hew=(16.273*10**-3)*hcw*(abs(P_w-P_g)/abs(T_w-T_g));
    #print(hew)
    return hew
def h1w(eps_g,eps_w,T_w,T_g,C,n,d_f):
    v_1=hrw(eps_g,eps_w,T_w,T_g)
    v_2=hcw_newCN(C,n,T_w,T_g,d_f)
    v_3=hew(C,n,T_w,T_g,d_f)
    h1w=(v_1+v_2+v_3)
    return h1w
...
def fract(eps_g,eps_w,T_w,T_g,C,n,d_f):
    hcw_v=hcw_newCN(C,n,T_w,T_g,d_f)
    hew_v=hew(C,n,T_w,T_g,d_f)
    hrw_v=hrw(eps_g,eps_w,T_w,T_g)
    h1w_v= h1w(eps_g,eps_w,T_w,T_g,C,n,d_f);
...
def eff_ins(T_w,T_g,A_s,I_t,C,n,d_f):
    ...
    return eff_int
def eff_jesus(eps_g,eps_w,T_w,T_g,Ta,V,C,n,d_f):
    h1w_v=h1w(eps_g,eps_w,T_w,T_g,C,n,d_f);
    h1g_v=h1g(eps_g,T_g,Ta,V);
    eff_int=((h1w_v*h1g_v)/(h1w_v+h1g_v))*abs(T_w-T_g)
    return eff_int

def exergy(T_w,T_g,A_s,Ta,I_t,C,n,d_f):
    T_s=5505;
    hew_v=hew(C,n,T_w,T_g,d_f);
...
import math
def T_w_new(alpha_w,alpha_g,alpha_b,R_g,R_w,eps_g,eps_w,T_w,T_g,V_basin,h_wbslc,K_i,L_i,I_t,Ta,C,n,d_f):
...
Sheet1=wb2 [sheets[0]]
import time
T_w=(sheet.cell_value(1, 3))
T_g=(sheet.cell_value(1, 4))
...
##    print(sheet.ncols)
    ...
    P_w_val=PT(T_w);
    P_g_val=PT(T_g);
    hew_val=hew(C,n,T_w,T_g,d_f);
    hcw_val=hcw_newCN(C,n,T_w,T_g,d_f);
    hrw_val=hrw(eps_g,eps_w,T_w,T_g);
    h1w_val=h1w(eps_g,eps_w,T_w,T_g,C,n,d_f);
    qcw,qew,qrw,Qtotal_val,Few_val,Fcw_val,Frw_val=fract(eps_g,eps_w,T_w,T_g,C,n,d_f);
    yield_val=mew(T_w,T_g,C,n,d_f);
    Eff_val=eff_ins(T_w,T_g,A_s,I_t,C,n,d_f);
    Exergy_val= exergy(T_w,T_g,A_s,Ta,I_t,C,n,d_f)
    T_w_new_v,T_g_new_v=T_w_new(alpha_w,alpha_g,alpha_b,R_g,R_w,eps_g,eps_w,T_w,T_g,V_basin,h_wbslc,K_i,L_i,I_t,Ta,C,n,d_f)
