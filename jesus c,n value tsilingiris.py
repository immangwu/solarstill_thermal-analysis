...
def latentheatofvapor(Temp):
    ...
    L=(2503.943143-2.451556893*Temp)*10**3
    return L
...
import numpy as np
def PT(Temp):
    PT = (1.131439334+((-3.750393331*10**-2)*Temp)+((5.591559189*10**-3)*Temp**2)+((-6.220459433*10**-5)*Temp**3)+((1.10581611*10**-6)*Temp**4))*10**3
    return PT
def TPP(Temp,T_w,T_g,K_i,d_f ,m_ew): #thermo physical properties
    density = ((-5.760867827*10**-7)*Temp**3)+((4.697926602*10**-5)*Temp**2)+((-6.043625845*10**-3)*Temp)+1.299995662;  #Toyama etal 1987
    
    SHC = (((5.122303796*10**-8)*Temp**4)+((-7.898561559*10**-6)*Temp**3)+((4.769110559*10**-4)*Temp**2)+(-0.01057758092*Temp)+1.088022802)*10**3; # specific heat capacity
    TC_K=((-9.489325324*10**-9)*Temp**3)+((4.631207189*10**-7)*Temp**2)+((5.526004579*10**-5)*Temp)+0.02416826077; # thermal Conductivity
    viscos=((-2.644372665*10**-13)*Temp**4)+((3.4139225*10**-11)*Temp**3)+((-2.162762*10**-9)*Temp**2)+((9.151853945*10**-8)*Temp)+(1.6857317*10**-5)
